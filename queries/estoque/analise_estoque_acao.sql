...
SELECT
    ...
    CASE
        ...
        WHEN DATEDIFF(DAY FROM tbDataUltimaentrada.data TO CURRENT_DATE) <= 90  THEN 'ANALISE PARA RECOMPRA'
        WHEN DATEDIFF(DAY FROM tbDataUltimaentrada.data TO CURRENT_DATE) <= 180 THEN 'ACOMPANHAMENTO'
        WHEN DATEDIFF(DAY FROM tbDataUltimaentrada.data TO CURRENT_DATE) <= 270 THEN 'PROMOCAO 20%'
        WHEN DATEDIFF(DAY FROM tbDataUltimaentrada.data TO CURRENT_DATE) <= 360 THEN 'LIQUIDA 30%'
        WHEN DATEDIFF(DAY FROM tbDataUltimaentrada.data TO CURRENT_DATE) <= 720 THEN 'LIQUIDA 50%'
        WHEN DATEDIFF(DAY FROM tbDataUltimaentrada.data TO CURRENT_DATE) > 720  THEN 'ACAO ESPECIAL'
