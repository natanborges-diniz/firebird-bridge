-- queries/vendas/formas_pagamento_auditoria_light.sql
-- Auditoria leve de vendas por forma de pagamento (vendas normais)
-- Parâmetros (7):
--   1) empresa (int)
...
itens_por_transacao AS (
  SELECT
    ...
    SUM(COALESCE(ti.TOTAL, 0) - COALESCE(ti.VALORDESCONTO, 0) - COALESCE(ti.TOTALIPI, 0)) AS TOTAL_VENDIDO
