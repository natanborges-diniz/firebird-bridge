...
FROM
  ...
  JOIN NATUREZAOPERACAO no
    ON ti.COD_NATUREZAOPERACAO = no.COD_NATUREZAOPERACAO
  JOIN SAIDA s
...
WHERE
  no.TIPO = 1
  AND t.DATAENCERRAMENTO BETWEEN :DATA_INICIAL AND :DATA_FINAL
  AND (:COD_EMPRESA IS NULL OR t.COD_EMPRESAESTOQUE = :COD_EMPRESA)
GROUP BY
