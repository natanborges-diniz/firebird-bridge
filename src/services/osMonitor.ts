...
const FIREBIRD_BRIDGE_BASE_URL =
  process.env.NEXT_PUBLIC_FIREBIRD_BRIDGE_URL ||
...
export async function getOsMonitor(
  ...
  if (Array.isArray(data)) return data as OsMonitorItem[];
  if (Array.isArray(data.data)) return data.data as OsMonitorItem[];
